Vue.component("group-trainings", {
	name: "group-trainings",
	data: function () {
		    return {
				myGroupTrainingsHistory : null,
...
` 
	<v-row>
		...
		<v-card class="mx-auto mt-8" outlined rounded="8">
				<v-row v-for="th in myGroupTrainingsHistory" :key="th.id">
...
`
	...
	methods : {
		...
		filterTrainingHistory(){
			this.myGroupTrainingsHistory = JSON.parse(JSON.stringify(this.trainingHistoryBackup));
		...
		filterTrainingHistoryBySportFacility(){
			...
			this.myGroupTrainingsHistory = this.myGroupTrainingsHistory.filter(th => {
		...
		sortTrainingHistoryBySportsFacilityName(){
			this.myGroupTrainingsHistory.sort((a, b) => a.training.sportsFacility.name.localeCompare(b.training.sportsFacility.name));
			if(!this.sortSportsFacilityNameAsc)this.myGroupTrainingsHistory.reverse();
			
			this.sortSportsFacilityNameAsc = !this.sortSportsFacilityNameAsc;
		},
		sortTrainingHistoryByAdditionalPrice(){
			this.myGroupTrainingsHistory.sort((a, b) => {return a.training.additionalPrice - b.training.additionalPrice});
			if(!this.sortAdditionalPriceAsc)this.myGroupTrainingsHistory.reverse();
			
			this.sortAdditionalPriceAsc = !this.sortAdditionalPriceAsc;
		},
		sortTrainingHistoryByDate() {
			this.myGroupTrainingsHistory.sort((a, b) => {return a.date - b.date});
			if(!this.sortAdditionalPriceAsc)this.myGroupTrainingsHistory.reverse();
	...
	mounted () {
        axios.get('rest/TrainingHistoryController/getAllTrainingsHistoryByCertainCoach/' + this.userToken.id + '/2')
	          .then(response => {
					this.myGroupTrainingsHistory = response.data;
					this.myGroupTrainingsHistory.forEach(th =>{
					...
					this.trainingHistoryBackup = this.myGroupTrainingsHistory;
					this.range[1] = this.max;
				})
	          .catch(error => {
