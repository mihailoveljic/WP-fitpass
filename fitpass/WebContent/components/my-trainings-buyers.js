					...
					<v-card width="100%" class="mx-3" flat outlined>
						<v-row class="d-flex">
							...
							<v-col cols="6" align-self="center">
								...
								<div class="text-h6 gray--text text-center mt-2">
									{{ th.training.trainingType }}
								</div>
							...
							<v-col cols="3" class="ma-auto" align-self="center">
								...
								<div class="text-subtitle-1 gray--text text-center">
									{{ th.training.sportsFacility.sportsFacilityType }}
								</div>
								<v-divider></v-divider>
								<div class="text-body-1 gray--text text-center">
...
`
	...
	methods : {
		...
		filterTrainingHistory(){
			this.trainingHistory = JSON.parse(JSON.stringify(this.trainingHistoryBackup));
		...
		sortTrainingHistoryBySportsFacilityName(){
			this.trainingHistory.sort((a, b) => a.training.sportsFacility.name.localeCompare(b.training.sportsFacility.name));
			if(!this.sortSportsFacilityNameAsc)this.trainingHistory.reverse();
			
			this.sortSportsFacilityNameAsc = !this.sortSportsFacilityNameAsc;
		},
		sortTrainingHistoryByAdditionalPrice(){
			this.trainingHistory.sort((a, b) => {return a.training.additionalPrice - b.training.additionalPrice});
			if(!this.sortAdditionalPriceAsc)this.trainingHistory.reverse();
			
			this.sortAdditionalPriceAsc = !this.sortAdditionalPriceAsc;
		},
		sortTrainingHistoryByDate() {
			this.trainingHistory.sort((a, b) => {return a.date - b.date});
			if(!this.sortAdditionalPriceAsc)this.trainingHistory.reverse();
