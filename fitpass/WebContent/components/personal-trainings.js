Vue.component("personal-trainings", {
	...
	data: function () {
		    return {
				myPersonalTrainingsHistory : null,
...
`
	...
	methods : {
		...
		filterTrainingHistory(){
			this.myPersonalTrainingsHistory = JSON.parse(JSON.stringify(this.trainingHistoryBackup));
		...
		filterTrainingHistoryBySportFacility(){
			...
			this.myPersonalTrainingsHistory = this.myPersonalTrainingsHistory.filter(th => {
		...
		sortTrainingHistoryBySportsFacilityName(){
			this.myPersonalTrainingsHistory.sort((a, b) => a.training.sportsFacility.name.localeCompare(b.training.sportsFacility.name));
			if(!this.sortSportsFacilityNameAsc)this.myPersonalTrainingsHistory.reverse();
			
			this.sortSportsFacilityNameAsc = !this.sortSportsFacilityNameAsc;
		},
		sortTrainingHistoryByAdditionalPrice(){
			this.myPersonalTrainingsHistory.sort((a, b) => {return a.training.additionalPrice - b.training.additionalPrice});
			if(!this.sortAdditionalPriceAsc)this.myPersonalTrainingsHistory.reverse();
			
			this.sortAdditionalPriceAsc = !this.sortAdditionalPriceAsc;
		},
		sortTrainingHistoryByDate() {
			this.myPersonalTrainingsHistory.sort((a, b) => {return a.date - b.date});
			if(!this.sortAdditionalPriceAsc)this.myPersonalTrainingsHistory.reverse();
	              ...
	              .then(response => {
					if(response.data == true){
						axios.get('rest/TrainingHistoryController/getAllTrainingsHistoryByCertainCoach/' + this.userToken.id + '/1')
							...
							this.trainingHistoryBackup = this.myPersonalTrainingsHistory;
							this.range[1] = this.max;
						})
	              		.catch(error => {
